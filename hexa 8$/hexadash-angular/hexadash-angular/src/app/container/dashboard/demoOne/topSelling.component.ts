...
@Component({
  ...
  <div class="px-[25px] text-dark dark:text-white/[.87] font-medium text-[17px] flex flex-wrap items-center justify-between max-sm:flex-col max-sm:h-auto max-sm:mb-[15px]">
    <h1 class="mb-0 inline-flex items-center py-[16px] max-sm:pb-[5px] overflow-hidden whitespace-nowrap text-ellipsis text-[18px] font-semibold text-dark dark:text-white/[.87]"> Top Selling Product</h1>
        ...
        <nz-table [nzData]="products[tab.key]" [nzFrontPagination]="false" [nzShowPagination]="false" class="text-sm rounded-[5px]">
          ...
          <tbody class="bg-white dark:bg-[#1b1d2a]">
            <tr *ngFor="let product of products[tab.key]" class="group">
              ...
              <td class="px-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none dark:group-hover:bg-white/10">{{ product.price }}</td>
              <td class="px-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none dark:group-hover:bg-white/10">{{ product.sold }}</td>
              <td class="ps-4 pe-4 py-2.5 font-normal last:text-end capitalize text-[14px] text-dark dark:text-white/[.87] border-none dark:group-hover:bg-white/10 rounded-e-[4px]">{{ product.revenue }}</td>
            </tr>
          </tbody>
        </nz-table>
      </div>
