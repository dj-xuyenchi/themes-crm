...
const inlineSvg = function () {
  document.querySelectorAll('img.svg').forEach(function (e: Element) {
    const imgID: any = e.getAttribute('id');
...
const chartLinearGradient = (canvas: any, height: number, color: any) => {
...
// Custom Tooltip
const customTooltips = function (this: any, context: any) {
  // Tooltip Element
  let tooltipEl: HTMLDivElement = document.querySelector('.chartjs-tooltip')!;
  if (!this._chart.canvas.closest('.ninjadash-chart-container')!.contains(tooltipEl)) {
    tooltipEl = document.createElement('div');
    tooltipEl.className = 'chartjs-tooltip';
    tooltipEl.innerHTML = '<table></table>';

    document.querySelectorAll('.ninjadash-chart-container').forEach((el: Element) => {
